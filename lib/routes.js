	  	...
	  	if(error)
	  	{
	  		...
	  		if(accident)
	  		{
	  			...
	  			if(user){
		  			...
		  			message = "Accident at " + address + " of user " + profile.name + ". Hurry!!! \nhttp://maps.google.com/maps?q=loc:" + lat + "," + longt + " (Accident Site)";
		  			numbers = profile.personalNumbers;
		  			numbers.forEach(function(no){
		  				Meteor.call('sendSMS',no,message);
		  			});
